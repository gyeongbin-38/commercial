// Clean showcase video for Upwork Project Catalog.
// No URLs, no emails anywhere: slates show name/kind only, ending card
// points contact to Upwork messages (on-platform = review-safe).
// Scroll caps at ~80% of page so footers/legal strips never appear.
// Usage: node scripts/showcase-video-clean.mjs  (produces video/showcase-clean.webm)
...
const scrollThrough = async (ms) => {
  ...
  // Cap at 80% of scrollable height so footers never enter frame.
  await page.evaluate(async (dur) => {
    const max = Math.min(
      (document.body.scrollHeight - innerHeight) * 0.8,
      innerHeight * 5
    );
...
// End card — on-platform contact CTA, no email/URL
await page.goto(
  slate(
    "Fixed price · 5-10 day delivery",
    "Let's build yours.",
    "Message me on Upwork — I reply within one business day."
  )
);
await hold(4000);
...
copyFileSync(src, `${outDir}/showcase-clean.webm`);
console.log(`recorded: ${outDir}/showcase-clean.webm`);
